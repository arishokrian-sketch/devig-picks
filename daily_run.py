...
from devig_model import (
    ...
    DEMO_ELO_RESULTS, SPORT_KEYS, fetch_mlb_season_results,
)
 
MIN_EV = float(os.environ.get("MIN_EV", "0.03"))
TOP_N = 5
SEASON_START = os.environ.get("SEASON_START", "2026-03-15")  # opening day, adjust if needed
 
 
def load_results_for_sport(sport: str) -> list[dict]:
    """
    For MLB, automatically pulls the real season-to-date from MLB's public API —
    no manual file needed, and it's always current as of this morning's run.
    For other sports (no free live-results API wired up yet), falls back to
    results_data/{sport}.json if you've created one, otherwise bundled demo data.
    """
    if sport == "mlb":
        today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
        results = fetch_mlb_season_results(SEASON_START, today)
        if results:
            print(f"[info] Loaded {len(results)} real completed MLB games "
                  f"({SEASON_START} to {today}) for the Elo model.")
            return [{"home": g["home"], "away": g["away"],
                     "home_score": g["home_score"], "away_score": g["away_score"]}
                    for g in results]
        print("[warn] Live MLB results fetch returned nothing — falling back to demo data. "
              "Picks today will not reflect real team strength.")
